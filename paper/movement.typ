...
Parameter settings were validated using a dedicated stress-test routine (`stresstest.py`) designed to exercise the pick-and-place routine under representative operating conditions. The test executes a fixed sequence of 20 `ned2.move` operations, covering (i) repeated stack-to-board placements, (ii) repeated board-to-stack removals up to the maximum expected stack height, and (iii) additional board-to-board transfers for coverage. Throughout the procedure, the end-effector follows the approach-from-above motion sequence (@algorithm), thereby stress-testing the routine across realistic pick-and-place scenarios. The routine was used iteratively to tune the grasp offsets and the nominal `chip_height` parameters in `niryo_config.toml` until stable pickup and placement were achieved without dropping pieces or destabilizing stacks.
